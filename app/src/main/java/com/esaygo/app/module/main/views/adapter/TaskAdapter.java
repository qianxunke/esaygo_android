...
public class TaskAdapter extends BaseQuickAdapter<TaskDetails, BaseViewHolder> {
    ...
    public TaskAdapter(int layoutResId, List<TaskDetails> data, BookFragment bookFragment) {
    ...
    protected void convert(BaseViewHolder helper, TaskDetails item) {
        ...
        if (item.getTask().getStatus() == 0) {
            ...
            helper.setVisible(R.id.tx_local_ticket,false);
        } else if (item.getTask().getStatus() == 1 || item.getTask().getStatus() == 2 || item.getTask().getStatus() == 4||item.getTask().getStatus() == 6) {
            helper.setText(R.id.tx_status, item.getTask().getStatus() == 4 ? "待抢票" : "抢票中");
            helper.setText(R.id.tx_action, "取消抢票");
            helper.setVisible(R.id.tx_action, true);
            helper.setVisible(R.id.tx_local_ticket,true);
        ...
        helper.setOnClickListener(R.id.tx_local_ticket, new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                v.setEnabled(false);
                bookFragment.startTicke(item);
                v.setEnabled(true);
            }
        });
    }
